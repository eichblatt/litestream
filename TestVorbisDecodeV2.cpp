...
uint32_t            m_sampleRate=SAMPLE_RATE;
uint8_t             m_bitsPerSample = 16;           // bitsPerSample
uint8_t             m_vol=64;                       // volume
size_t              m_i2s_bytesWritten = 0;         // set in i2s_write() but not used
uint8_t             m_channels=CHANNELS;
...
void setupI2S()
{
    ...
    m_i2s_config.dma_buf_count        = 8;      // 8 = max buffers
    m_i2s_config.dma_buf_len          = 1024;   // 1024 = max value
...
void setup() 
{
    ...
    // tell the httpClient to collect the Content-Length header when it makes the request
    ...
    // Get the Content-Length header to see the length of the file. We must tell the httpClient to collect that header above
    int len = httpClient.header("Content-Length").toInt();
    ...
    // Allocate a buffer to store data streaming in from WiFi
    ...
    // Read data from the HTTP response until the decoder has enough to start or the buffer fills up
    while (header_pos < BUFFER_SIZE && httpClient.connected())
    {
        bytes_read = httpClient.getStreamPtr()->read(buffer + header_pos, BUFFER_SIZE - header_pos);
        Serial.printf("Read %d bytes\n", bytes_read);
        header_pos += bytes_read;

        // Try and read what we have so far and start the dcoder. Will fail with VORBIS_need_more_data if there isn't enough data in the buffer yet
        v = stb_vorbis_open_pushdata(buffer, header_pos, &used, &error, &VorbisBuffer);
        
        if (error == VORBIS_need_more_data)
        {
          Serial.println("Need more data");
          continue;
        }
    ...
    int bytes_in_buffer = header_pos - used;
    ...
    for(;;)
    {
        ...
        {
            Serial.printf("*");
            bytes_read = httpClient.getStreamPtr()->read(buffer + bytes_in_buffer, BUFFER_SIZE - bytes_in_buffer);
