...
class Display(object):
    ...
    def is_off_grid(self, xmin, ymin, xmax, ymax):
        ...
        if ymin < 0:
            ...
            return True
        if xmax >= self.width:
            print("x-coordinate: {0} above maximum of {1}.".format(xmax, self.width - 1))
            return True
        if ymax >= self.height:
            print("y-coordinate: {0} above maximum of {1}.".format(ymax, self.height - 1))
            return True
        return False
    ...
    def set_scroll(self, top, bottom):
        ...
        if top + bottom <= self.height:
            ...
            self.write_cmd(self.VSCRDEF, top >> 8, top & 0xFF, middle >> 8, middle & 0xFF, bottom >> 8, bottom & 0xFF)
    ...
    def read_data(self, nbytes, write=0x00):
        ...
        try:
            self.cs(0)
            rxdata = self.spi.read(nbytes, write)
        finally:
            self.cs(1)
        return rxdata
