...
def main_loop(col_dict):
    ...
    while True:
        ...
        if pPower_old != controls.pPower.value():
            ...
            if pPower_old:
                controls.tft.disp.fill_circle(5 + 8, 108 + 8, 8, controls.st7789.BLUE)
                print("Power UP")
            else:
                controls.PowerLED = not controls.PowerLED
                controls.tft.disp.fill_circle(5 + 8, 108 + 8, 8, controls.st7789.WHITE)
                print("Power DOWN")

        if pSelect_old != controls.pSelect.value():
            pSelect_old = controls.pSelect.value()
            if pSelect_old:
                controls.tft.disp.rect(105, 108, 16, 16, controls.st7789.BLUE)
                print("Select UP")
            else:
                controls.tft.disp.rect(105, 108, 16, 16, controls.st7789.WHITE)
                print("Select DOWN")

        if pPlayPause_old != controls.pPlayPause.value():
            pPlayPause_old = controls.pPlayPause.value()
            if pPlayPause_old:
                controls.tft.disp.fill_polygon(controls.PlayPausePoly, 130, 108, controls.st7789.BLUE)
                print("PlayPause UP")
            else:
                controls.tft.disp.fill_polygon(controls.PlayPausePoly, 130, 108, controls.st7789.WHITE)
                print("PlayPause DOWN")

        if pStop_old != controls.pStop.value():
            pStop_old = controls.pStop.value()
            if pStop_old:
                controls.tft.disp.fill_rect(55, 108, 16, 16, controls.st7789.BLUE)
                print("Stop UP")
            else:
                controls.tft.disp.fill_rect(55, 108, 16, 16, controls.st7789.WHITE)
                print("Stop DOWN")

        if pRewind_old != controls.pRewind.value():
            pRewind_old = controls.pRewind.value()
            if pRewind_old:
                controls.tft.disp.fill_polygon(controls.RewPoly, 30, 108, controls.st7789.BLUE)
                print("Rewind UP")
            else:
                controls.tft.disp.fill_polygon(controls.RewPoly, 30, 108, controls.st7789.WHITE)
                print("Rewind DOWN")

        if pFFwd_old != controls.pFFwd.value():
            pFFwd_old = controls.pFFwd.value()
            if pFFwd_old:
                controls.tft.disp.fill_polygon(controls.FFPoly, 80, 108, controls.st7789.BLUE)
                print("FFwd UP")
            else:
                controls.tft.disp.fill_polygon(controls.FFPoly, 80, 108, controls.st7789.WHITE)
                print("FFwd DOWN")

        if pYSw_old != controls.pYSw.value():
            pYSw_old = controls.pYSw.value()
            if pYSw_old:
                controls.tft.disp.text(controls.sfont, "Y", 60, 70, controls.st7789.WHITE, controls.st7789.BLUE)
                print("Year UP")
            else:
                # cycle through Today In History (once we know what today is!)
                controls.tft.disp.text(controls.sfont, "Y", 60, 70, controls.st7789.BLUE, controls.st7789.WHITE)
                print("Year DOWN")

        if pMSw_old != controls.pMSw.value():
            pMSw_old = controls.pMSw.value()
            if pMSw_old:
                controls.tft.disp.text(controls.sfont, "M", 0, 70, controls.st7789.WHITE, controls.st7789.BLUE)
                print("Month UP")
            else:
                controls.tft.disp.text(controls.sfont, "M", 0, 70, controls.st7789.BLUE, controls.st7789.WHITE)
                print("Month DOWN")

        if pDSw_old != controls.pDSw.value():
            pDSw_old = controls.pDSw.value()
            if pDSw_old:
                controls.tft.disp.text(controls.sfont, "D", 30, 70, controls.st7789.WHITE, controls.st7789.BLUE)
                print("Day UP")
            else:
                for date in sorted(col_dict["GratefulDead"].keys()):
                    if date > key_date:
                        ...
                        break
                controls.tft.disp.text(controls.sfont, "D", 30, 70, controls.st7789.BLUE, controls.st7789.WHITE)
                print("Day DOWN")
        ...
        if year_old != year_new:
            year_old = year_new
            # controls.tft.disp.text(controls.font, f"{year_new%100}", 90, 0, stage_date_color, controls.st7789.BLACK)
            controls.tft.show_staged_date(f"{date_new}")
            print("year =", year_new)

        if month_old != month_new:
            month_old = month_new
            controls.tft.disp.text(controls.font, f"{month_new:2d}-", 0, 0, stage_date_color, controls.st7789.BLACK)
            print("month =", month_new)

        if day_old != day_new:
            day_old = day_new
            controls.tft.disp.text(controls.font, f"{day_new:2d}-", 43, 0, stage_date_color, controls.st7789.BLACK)
            print("day =", day_new)
        ...
        if date_old != date_new:
            ...
            try:
                ...
                print(vcs)
                controls.tft.clear_area(controls.tft.venue_bbox)
                controls.tft.show_venue(f"{vcs}")
...
def main():
    ...
    """
    controls.tft.disp.draw(controls.gothic_font, f"Grateful Dead", 0, 30, controls.st7789.color565(255, 255, 0), 0.8)
    controls.tft.disp.draw(controls.gothic_font, f"Time Machine", 0, 60, controls.st7789.color565(255, 255, 0), 0.8)
    controls.tft.disp.draw(controls.gothic_font, f"Loading", 0, 90, controls.st7789.color565(255, 255, 0), 1)
    col_dict = {}
