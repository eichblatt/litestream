...
_HEADER = (
    ...
    b"\x00\x00\x00\x00data\x00\x00\x00\x00"
)  # Template.
...
class VS1053:
    ...
    def _write_reg2(self, addr, value):  # Datasheet 7.4
        self._wait_ready()
    ...
    def soft_reset(self):
        ...
        a = self._read_reg(_SCI_CLOCKF)
        if a != 0x8800:
            raise OSError(f"No VS1053 device found. read {a}")
    ...
    def SCI_test(self):
        self.soft_reset()
        self.mode_set(_SM_TESTS)
        # Copy the mode to _SCI_HDAT0
        self.write(b"\x53\x70\xee\x00\x00\x00\x00\x00")
        time.sleep(2)
        mem_result = self._read_reg(_SCI_HDAT0)
        print(f"SCI Test Returned {hex(mem_result)}")
        assert self.mode() == mem_result, f"test_results {hex(mem_result)} do not match mode {hex(self.mode())}"
        self.mode_clear(_SM_TESTS)
    ...
    def memory_test(self):
        ...
        self.mode_set(_SM_TESTS)
        # 0x66-> Sample rate 22050 * 6/128 = 1034Hz 0x63->517Hz
        self.write(b"\x4D\xEA\x6D\x54\0\0\0\0")
        time.sleep(2)
        mem_result = self._read_reg(_SCI_HDAT0)
        print(f"Memory Test Returned {hex(mem_result)}")
        self.mode_clear(_SM_TESTS)
