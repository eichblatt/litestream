...
# If the space between readPos and the end of the buffer is less than OverflowSize then (upon read) we copy data from the end of the buffer to the overflow area so that the mp3/Vorbis frame is always completed.
#
# Notes:
# 1) We only ever write to the main buffer area. The buffer itself handles copying data to the overflow area when required, transparently to the reader of the buffer
...
class InRingBuffer:
    ...
    def get_write_available(self):  
        if self._writePos > self._readPos:
            BytesInOverflow = max(self.OverflowSize - self._readPos, 0) # If the read_pos is within the overflow area return the num bytes in there, else zero
    ...
    # Tell the buffer how many bytes we just read.  Must call this after every read from the buffer, and must call get_read_available() before this to set the readpointer correctly
...
# For the buffer between the decoder and the I2S output we use a Ring Buffer. Because the decoder must always be able to write one full frame (1024 samples = 4096 bytes at 16 bit stereo) we only return get_write_available if we have at least 5000 bytes available.
# The observed biggest number of samples returned is 4608 from MP3 files, so make it 5000 for some headroom
...
class AudioPlayer:
    ...
    def play_chunk(self):
        ...
        if BytesToPlay == 0:
            if self.FinishedDecoding: # End of playlist
                ...
                self.PlayLoopRunning = False # Set this flag to re-start the player loop when the decoder has generated enough data
                return
    ...
    def audio_pump(self):
            ...
            if (BytesAvailable := self.InBuffer.get_write_available()) > 0:
                try:  # We can get an exception here if we pause too long and the underlying socket gets closed
                    ...
                    if data is not None:
                        ...
                        if data == 0:  # Peer closed socket. This can be because of End-of-stream or it can happen in a long pause before our socket closes
                            gc.collect()
                            # End of track
        ...
        if self.AtTrackStart:
            ...
            if self.ID3Tag_size == 0: # We're at the start of a new track. Work out the size of the ID3 tag (if any) at the beginning
                self.current_track_bytes_decoded = 0
                readpos = self.InBuffer.get_readPos()
                if self.InBuffer.Bytes[readpos] == ord(b"I") and self.InBuffer.Bytes[readpos + 1] == ord(b'D') and self.InBuffer.Bytes[readpos + 2] == ord(b'3'): # ID3 tag at beginning. Work out the size
                    self.ID3Tag_size = 0
            ...
            if self.ID3Tag_size > 0: # There are not enough bytes in the buffer to skip all of them. Catch them next time we enter audio_pump
                return
            else:
        ...
        if self.InHeader:
            ...
            if self.Decoder == self.MP3:
                FoundSyncWordAt = MP3Player.MP3_Start(self.InBuffer.Buffer[self.InBuffer.get_readPos() :], self.InBuffer.get_read_available())
            elif self.Decoder == self.OGGVORBIS:
                FoundSyncWordAt = VorbisPlayer.Vorbis_Start(self.InBuffer.Buffer[self.InBuffer.get_readPos() :], self.InBuffer.get_read_available())
        ...
        Counter = 0  # Just for debugging. See how many times we run the loop before returning
        while True:
            # Do we have at least 5000 bytes available for the decoder to write to? If not we return and wait for the play loop to free up some space. Must match buffer overflowsize
            ...
            if self.Decoder == self.MP3:
                Result, BytesLeft, AudioSamples = MP3Player.MP3_Decode(self.InBuffer.Buffer[self.InBuffer.get_readPos() :], InBytesAvailable, self.OutBuffer.Buffer[self.OutBuffer.get_writePos() :])
            elif self.Decoder == self.OGGVORBIS:
                Result, BytesLeft, AudioSamples = VorbisPlayer.Vorbis_Decode(self.InBuffer.Buffer[self.InBuffer.get_readPos() :], InBytesAvailable, self.OutBuffer.Buffer[self.OutBuffer.get_writePos() :])
            ...
            if Result == 0 or Result == 100 or Result == 110:
                ...
                if self.InBuffer.Bytes[pos] == ord(b"T") and self.InBuffer.Bytes[pos + 1] == ord(b'A') and self.InBuffer.Bytes[pos + 2] == ord(b'G'): # ID3 tag at the end. Fixed size of 128 bytes. Skip it.
                    print("Skipping TAG")
                    self.InBuffer.get_read_available()
                    self.InBuffer.bytes_wasRead(128) # // This is an V1.x id3tag after an audio block, ID3 v1 tags are ASCII. Version 1.x is a fixed size at the end of the file (128 bytes) after a TAG keyword.
